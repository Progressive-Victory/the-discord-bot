...
export const search_events = new ChatInputCommand({
	...
	execute: async (interaction) => {
		...
		if (interaction.isChatInputCommand()) {
			...
			const events = await findEventsMatchingQuery(interaction, interaction.guild);
	...
	autocomplete: async (interaction) => {
		if (interaction.isAutocomplete()) {
			...
			const all_events = interaction.guild ? await fast_fetch_events(interaction.guild) : []
			if (focus.name === "id" && all_events !== undefined) {
				...
				const filtered = all_events.filter(event =>
					event.name.startsWith(focus.value)
				).map(event => ({
					name: event.name,
					value: event.name
				}));
...
async function fast_fetch_events(guild:Guild):Promise<GuildScheduledEvent[]>{
	if (Date.now()-prevEventCacheTimestamp<CACHE_DURATION && guild.scheduledEvents.cache.size>0){
